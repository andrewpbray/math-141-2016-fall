...
information - the data. In this lab we explore flights, specifically all flights
that departed from the two major airports in the pacific northwest in 2013 (SEA and PDX).
We will generate simple graphical and numerical 
...
We begin by loading the `flights` data frame. Type the following in your console
to load the data:
...
- Which of the airport has a better on time percentage for 
departing flights?
...
This function says to plot the `dep_delay` variable from the `flights` data 
...
save the result as a new data frame called `flights`.
...
<div id="boxedtext">
**Logical operators: ** Filtering for certain observations (e.g. flights to a 
...
```{r lax-flights-summ}
lax_flights %>%
  summarise(mean_dd = mean(dep_delay, na.rm = TRUE), median_dd = median(dep_delay, na.rm = TRUE), n = n())
```
...
sfo_feb_flights %>%
  group_by(origin) %>%
  summarise(median_dd = median(dep_delay, na.rm = TRUE), iqr_dd = IQR(dep_delay, na.rm = TRUE), n = n())
```
...
1.  Which month has the highest average departure delay from a PNW airport? What
...
### On time departure rate for PNW airports

Suppose you will be flying out of the PNW and want to know which of the 
two major airports has the best on time departure rate of departing flights. 
...
flights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = mean(dep_type == "on time", na.rm = TRUE)) %>%
  arrange(desc(ot_dep_rate))
```

1.  If you were selecting an airport simply based on on time departure 
    percentage, which airport would you choose to fly out of?
...
```{r echo=FALSE, eval=TRUE, fig.width=7, fig.height=4, warning = FALSE}
data(flights)
dl_aa_ua <- flights %>%
  filter(carrier == "AA" | carrier == "DL" | carrier == "UA")
qplot(x = dep_delay, y = arr_delay, data = dl_aa_ua, color = carrier, alpha = I(.5))
```